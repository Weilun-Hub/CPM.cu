...
#include "eagle3_attn.cuh"

template <typename T>
struct Eagle3W4A16GPTQMarlinAttention {
    int hidden_size;
    ...
    Eagle3W4A16GPTQMarlinAttention(int hidden_size, int num_attention_heads, int num_key_value_heads, int head_dim, float rms_norm_eps, int group_size, int window_size = 0, bool use_qk_norm = false, bool use_attn_bias = false) {
        ...
        this->attn_norm = new Eagle3Norm<T>(hidden_size, rms_norm_eps);

        this->qkv_proj = new W4A16GPTQMarlinLinear<T>(2 * hidden_size, (num_attention_heads + 2 * num_key_value_heads) * head_dim, group_size, true, use_attn_bias);
        this->q_proj = new W4A16GPTQMarlinLinear<T>(2 * hidden_size, num_attention_heads * head_dim, group_size, true, use_attn_bias);
        this->k_proj = new W4A16GPTQMarlinLinear<T>(2 * hidden_size, num_key_value_heads * head_dim, group_size, true, use_attn_bias);
        this->v_proj = new W4A16GPTQMarlinLinear<T>(2 * hidden_size, num_key_value_heads * head_dim, group_size, true, use_attn_bias);
        this->o_proj = new W4A16GPTQMarlinLinear<T>(num_attention_heads * head_dim, hidden_size, group_size, true, false); // o_proj 不需要 bias
    ...
    void init_weight_ptr(Memory* memory) {
        ...
        this->k_proj->weight = this->q_proj->weight + 2 * hidden_size * this->num_attention_heads * this->head_dim;
        this->v_proj->weight = this->k_proj->weight + 2 * hidden_size * this->num_key_value_heads * this->head_dim;
    ...
    int64_t init_output_ptr(Memory* memory, int32_t num_tokens, int64_t offset) {
        ...
        int64_t qkv_permute_end = memory->allocate((void**)&this->permute_qkv_output, qk_norm_end, num_tokens * (this->num_attention_heads + 2*this->num_key_value_heads) * this->head_dim * sizeof(T));
    ...
    void prefill(const Stream& stream, int32_t num_tokens, int32_t num_history_tokens, T* hidden, T* embed, T* prev_hidden, int32_t* position_ids, KVCache<T>* kv_cache, T* a_tmp, float* c_tmp) {
        T* k_cache = kv_cache->offset_k(num_history_tokens);
        T* v_cache = kv_cache->offset_v(num_history_tokens);
        this->attn_norm->prefill(stream, num_tokens, hidden, embed, prev_hidden);
    ...
    void decode(const Stream& stream, int32_t num_tokens, int32_t padded_length, T* hidden, T* embed, T* prev_hidden, int32_t* position_ids, int32_t* cache_length, const Mask& mask, KVCache<T>* kv_cache, T* a_tmp, float* c_tmp) {
        this->attn_norm->prefill(stream, num_tokens, hidden, embed, prev_hidden);
        T *q, *k, *v;
        if (num_tokens > 1) {
            this->qkv_proj->prefill(stream, num_tokens, this->attn_norm->output, a_tmp, c_tmp);
