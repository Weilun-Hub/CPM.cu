...
struct Eagle3Layer {
    ...
    void prefill(int32_t num_tokens, int32_t num_history_tokens, T* hidden, T* embed, T* prev_hidden, int32_t* position_ids, KVCache<T>* kv_cache, T* prev_layer_states=nullptr) {
        ...
        this->attn->prefill(calc_stream, num_tokens, num_history_tokens, hidden, embed, prev_hidden, position_ids, kv_cache);
        ...
        this->ffn->prefill(calc_stream, num_tokens, hidden, this->attn->output);
    ...
    void decode(int32_t num_tokens, int32_t padded_length, T* hidden, T* embed, T* prev_hidden, int32_t* position_ids, int32_t* cache_length, const Mask& mask, KVCache<T>* kv_cache, T* prev_layer_states=nullptr) {
        ...
        this->attn->decode(calc_stream, num_tokens, padded_length, hidden, embed, prev_hidden, position_ids, cache_length, mask, kv_cache);
        ...
        this->ffn->decode(calc_stream, num_tokens, hidden, this->attn->output);
