...
#include "../minicpm4/minicpm4_model.cuh"
#include "../minicpm4/minicpm4_w4a16_gptq_marlin_model.cuh"
...
#include "eagle3_w4a16_gptq_marlin_layer.cuh"
...
struct Eagle3Impl : Model {
    ...
    Eagle3Impl(
        ...
        printf("[DEBUG ZWL] Eagle3Impl init, residual_scale: %f\n", this->residual_scale);
        ...
        if constexpr (std::is_same_v<LayerType, Eagle3W4A16GPTQMarlinLayer<T>>) {
            mid_layer = new Eagle3W4A16GPTQMarlinLayer<T>(this->model->hidden_size, intermediate_size, num_attention_heads, num_key_value_heads, head_dim, rms_norm_eps, group_size, this->residual_scale, eagle_window_size);
        } else {
            mid_layer = new Eagle3Layer<T>(this->model->hidden_size, intermediate_size, num_attention_heads, num_key_value_heads, head_dim, rms_norm_eps, this->residual_scale, eagle_window_size);
        }
    ...
    void eagle3_prefill(int num_history_tokens) {
        ...
        if constexpr (std::is_same_v<FcType, W4A16GPTQMarlinLinear<T>>) {
            this->fc->prefill(calc_stream, num_prev, this->model->eagle3_hidden, this->a_tmp, this->c_tmp);
        } else {
            this->fc->prefill(calc_stream, num_prev, this->model->eagle3_hidden);
        }
        T* layer_output = nullptr;
        this->mid_layer->prefill(num_prev, num_history_tokens, this->fc->output, this->prev_embed, layer_output, this->eagle_position_ids, this->kv_caches->caches[0]);
    ...
    void eagle3_decode(int32_t* cache_length) {
        ...
        T* layer_output = nullptr;
        this->mid_layer->decode(num_prev, this->eagle_padded_length, this->fc->output, this->prev_embed, layer_output, this->eagle_position_ids, cache_length, Mask(nullptr), this->kv_caches->caches[0]);
        layer_output = this->mid_layer->output;
        elementwise_scale(calc_stream, num_prev, this->model->hidden_size, layer_output, this->residual_scale);
        elementwise_add(calc_stream, num_prev, this->model->hidden_size, this->fc->output, layer_output, this->fc->output);
    ...
    void prefill(int32_t num_tokens, int32_t num_history_tokens, int32_t* input, int32_t* position_ids, void* output) {
        ...
        this->prev_hidden_state = this->model->eagle3_hidden;
    ...
    void draft(int32_t* tree_draft_ids, int32_t* tree_position_ids, int32_t* cache_length, uint64_t* tree_attn_mask, int32_t* tree_parent) {
        ...
        for (int d = 1; d < this->num_iter; ++d) {
            ...
            T* layer_output = nullptr;
            this->mid_layer->decode(topk_per_iter, this->eagle_padded_length, this->fc->output, this->model->embedding->output, layer_output, this->eagle_position_ids, this->eagle_cache_length, Mask(eagle_mask_2d, topk_per_iter, topk_per_iter * d), this->kv_caches->caches[0]);
            layer_output = this->mid_layer->output;
    ...
    int verify(int32_t num_tokens, int32_t* pred, int32_t* gt, int32_t* position_ids, int32_t* cache_length, uint64_t* mask_2d, int32_t* tree_parent) {
        ...
        remap_hidden(calc_stream, this->num_prev, this->model->hidden_size * 3, pred, this->model->eagle3_hidden, this->prev_hidden_state);
